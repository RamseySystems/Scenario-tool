...
def process_file(file, save_dir, upload_folder, standard_path):
    ...
    if file[-4:] == 'json':
        ...
        reformatted_with_implamentation = filter(unformatted, implementationGuidance=True)
        reformatted = filter(unformatted, implementationGuidance=False)

        save_obj_to_file(reformatted_with_implamentation, f'{save_dir}/{file[:-4]}_no_implamentation_reformatted.json')
        save_obj_to_file(reformatted, f'{save_dir}/{file[:-4]}_reformatted.json')
        ...
        sheets, group_alias_sheet, path_alias_sheet = open_worksheets(f'{upload_folder}/{file}')
        ...
        for sheet in sheets:
            if sheet.title == 'Story':
                ...
                story['summary'] = sheet.cell(row=summary_cell[0]+1, column=summary_cell[1]).value
                story['rationale'] = sheet.cell(row=rationale_cell[0]+1, column=rationale_cell[1]).value
                story['story'] = sheet.cell(row=story_cell[0]+1, column=story_cell[1]).value
                story['standard_url'] = sheet.cell(row=standard_url_cell[0]+1, column=standard_url_cell[1]).value
                story['standard_name'] = sheet.cell(row=standard_name_cell[0]+1, column=standard_name_cell[1]).value
                ...
                # generate path list
                path_list = gen_path_list(sheet, path_cell, value_cell, group_alias_sheet, path_alias_sheet)
                ...
                invalid_paths = validate_path_list(plain_path_list, standard_path_lists)
...
def open_worksheets(path: str):
    ...
    wb_array = []
    [wb_array.append(wb[ws]) for ws in sheet_names if ws.lower() not in ['group aliases', 'path aliases']]
    try:
...
def find_cell_location(sheet: dict, 
    ...
    for row in sheet.iter_rows():
        ...
        for cell in row:
            ...
            cell_index += 1
        row_index += 1
    raise ValueError(f'\'{cell_value}\' does not exist in worksheet: {sheet.title}')
    ...
    for cell in sheet.iter_rows(path_cell[0]+1, sheet.max_row, path_cell[1], value_cell[1]):
        if str(cell[0].value) != 'None':
            ...
            if in_loop:
                ...
                # get the alias name
                group_alias_name = path.split('.')[-1].replace('$$','').lower()

                # get the expanded list of paths
                group_alias_paths = expand_group_alias(group_alias_name, group_alias_sheet)
                ...
                for line in group_alias_paths:
                    ...
                    group_alias_paths_with_start.append(f'{no_group_alias}.{joined_line}')
                ...
                for line in group_alias_paths_with_start:
                    ...
                    for element in split_path:
                        if '$' in element:
                            expanded_element = expand_path_alias(path_alias_sheet, element)
                ...
                loop_lines = get_loop_lines(sheet, loop_start, path_cell, value_cell)
                expanded_loop_lines = expand_loop_lines(loop_lines, continuous_data, path_alias_sheet, group_alias_sheet)
        ...
        if str(cell[0].value) != 'None':
            ...
            for section in path_split:
                if '$$' in section:
                    ...
                    # get the alias name
                    group_alias_name = section.replace('$$','').strip().lower()
            ...
            if '$$' in path:
                ...
                for path in group_alias_paths:
                    if len(remainder_path) == 0:
                        ...
                        expanded_loop_lines.append([f'{remainder_path}.{path[0]}', path[1]])
            elif '$' in path:
                # get alias
                split_path = path.split('.')
                expanded_path = []
...
def expand_path_alias(path_alias_sheet: dict, 
    ...
    expanded_alias = path_alias_sheet.cell(row=alias_location[0], column=alias_location[1]+1).value
...
def get_standard_paths(standard_path: str):
    ...
    # get single_indent
    single_indent = get_whitespace(str(ws.cell(row=name_title_cell[0]+2, column=name_title_cell[1]).value))
    ...
    for row in ws.iter_rows(name_title_cell[0]+1, ws.max_row, name_title_cell[1], name_title_cell[1]):
        ...
        if line != 'None':
            ...
            stripped = re.sub(re.escape('\\xa0\\xa0\\xa0\\xa0'), r'', line).strip()
...
def create_object(start_paths: list):
    ...
    for path_lists in my_paths:
        if len(path_lists) == 1 and len(path_lists[0]) == 2:
            ...
            if len(matches) != 1:
                try:
                    result[path_lists[0][0].split("[")[0]].append(path_lists[0][1])
                except:
                    result.update({path_lists[0][0].split("[")[0]: [path_lists[0][1]]})
            else:
            ...
            if len(matches) != 1:
                try:
                    result[path_lists[0][0].split("[")[0]].append(create_object(trim_first_elements(path_lists)))
                except:
                    result.update({path_lists[0][0].split("[")[0]: [create_object(trim_first_elements(path_lists))]})
            else:
                try:
                    result.update({path_lists[0][0]: create_object(trim_first_elements(path_lists))})
                except:
                    return result
    return result
        ...
        for key in node:
            if key == 'desc':
                ...
                if implementationGuidance:
                    retVal['implementationGuidance'] = rem_new_line(re.sub(r'(?<=release=)[a-z][0-9]*', '', html.unescape(node[key][0]['#text'])).replace('&amp;','&'))
